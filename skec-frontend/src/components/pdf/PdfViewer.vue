<template>
  <div
    ...
    class="pdf-container flex flex-col h-screen bg-gray-900 select-none"
    @contextmenu.prevent
  >
    <!-- Controls bar -->
    <PdfControls
    ...
    <!-- PDF canvas area -->
    <div ref="viewerContainer" class="flex-1 overflow-y-auto flex flex-col items-center gap-4 py-6 px-4">
      <!-- Loading state -->
      ...
      <!-- Error -->
      <div v-else-if="loadError" class="flex items-center justify-center h-full text-white text-center">
        <div>
      ...
      <template v-else>
        ...
        >
          <canvas :ref="el => setCanvasRef(el, pageNum)" class="block" />
        </div>
      </template>
    </div>
  </div>
...
// Which pages to render (all pages for small docs, lazy for large)
...
async function loadPdf() {
  ...
  try {
    ...
    // Render all pages after DOM updates
...
async function zoomOut()   { zoom.value = Math.max(zoom.value - 0.25, 0.5); await rerenderAll() }
async function fitWidth()  {
  if (viewerContainer.value) {
    const containerW = viewerContainer.value.clientWidth - 64
    zoom.value = containerW / 800
  }
  await rerenderAll()
}
...
onUnmounted(async () => {
  ...
  try {
    await notesApi.logAccess(props.noteId, {
      action: 'closed',
